...
ROUTER_API.post("/login", (req, res)=>{
    const { username } = req.body
    if(!username){
        res.json({msg:"No se ha podido logear. Intentelo nuevamente", err:"Query username invalid or undefined."})
        return
    }
    req.session.username = username
    res.json({msg:`Hola ${username}. Logueado con exito!`})
})
